...
async function boundRegistration(ctx, customerId = "synthetic-customer-1") {
  ...
  await ctx.store.associateWebhook({
    eventId: newKey(), topic: "customers/create", payloadDigest: "a".repeat(64),
    email: registration.email_normalized, customerId, now: ctx.clock(),
  });
...
test("worker synchronizes by customer_id and keeps account pending review", async () => {
  ...
  const worker = new OutboxWorker({ store: ctx.store, shopifyClient: shopify, clock: ctx.clock, logger: ctx.logger, autoApprove: false });
...
test("Shopify failure schedules exponential retry with sanitized error", async () => {
  ...
  const worker = new OutboxWorker({ store: ctx.store, shopifyClient: shopify, clock: ctx.clock, logger: ctx.logger, maxAttempts: 3 });
  await worker.runOnce();
  const item = [...ctx.store.outbox.values()][0];
  assert.equal(item.attempts, 1);
  assert.equal(item.processed_at, null);
  assert.equal(item.last_error.includes("private-person"), false);
...
test("partial metafield/tag failure converges on retry", async () => {
  ...
  const worker = new OutboxWorker({ store: ctx.store, shopifyClient: shopify, clock: ctx.clock, logger: ctx.logger });
  await worker.runOnce();
